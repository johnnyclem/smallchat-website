...
- Nav links: What it does, Why it matters, Deep dive, Docs (→ /docs/intro), Get Started + GitHub icon
- Docs integration: Docusaurus docs site built and served as static files under `/docs/` path. Source in `docs-site/`, build output copied to `artifacts/web/public/docs/`. URLs centralized via `DOCS_BASE` (empty string for same-origin) and `GITHUB_URL` constants in Home.tsx. Vite plugin `serveDocsPlugin` in vite.config.ts handles serving docs HTML in dev mode.
